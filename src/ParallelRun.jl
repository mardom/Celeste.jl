...
function infer_box(box::BoundingBox, stagedir::String, outdir::String)
    ...
    nputs(nodeid, "timing: load_img=$(times.load_img/nthreads())")
    nputs(nodeid, "timing: load_cat=$(times.load_cat/nthreads())")
    nputs(nodeid, "timing: sched_ovh=$(times.sched_ovh/nthreads())")
    nputs(nodeid, "timing: ga_get=$(times.ga_get/nthreads())")
