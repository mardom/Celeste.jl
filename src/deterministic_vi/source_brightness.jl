...
function SourceBrightness{NumType <: Number}(
    ...
    for i = 1:Ia
        ...
        for b = 1:B
            E_l_a[b, i] = SensitiveFloat{NumType}(
                              length(BrightnessParams), 1,
...
function load_source_brightnesses{NumType <: Number}(
    ...
    for s in 1:ea.S
        is_active = s in ea.active_sources
        sbs[s] = SourceBrightness(ea.vp[s],
            calculate_derivs=(calculate_derivs && is_active),
            calculate_hessian=(calculate_hessian && is_active))
    end
    sbs
end
